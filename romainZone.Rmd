...
{
  if(!dir.exists(paste0("~/RDS_kappaExperiment/kappa",ikappa))) dir.create(paste0("~/RDS_kappaExperiment/kappa",ikappa))
          params <- evoParams(no_sp = 9,kappa = ikappa)
     for(iSim in 1:10)
     {
       sim <- evoProject(params = params, mutation = 0, alien = 5, t_max = 1500)
       saveRDS(sim, file = paste0("~/RDS_kappaExperiment/kappa",ikappa,"/sim",iSim,".RDS"))
     }
}
...
gamma is generated extremly low for invading species. Why?

```{r}

avail_energy <- getEncounter(params)[, length(params@w)]/params@w[length(params@w)]^(2 + 
            params@species_params$q - params@resource_params$lambda)
        gamma_default <- (species_params$h/avail_energy) * (species_params$f0/(1 - 
            species_params$f0))

a <- getEncounter(sim@params)

f <- get(sim@params@rates_funcs$Encounter)
```
...
sim_test <- evoProject(test, alien = 5)
